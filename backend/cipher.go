...
func SubBytesSubstitutionArr(arr []string) []string {
...
func InverseSubBytesSubstitutionArr(arr []string) []string {
...
func SubBytesSubstitution(value string) string {
    ...
    valueInt := hexToInt(value)
    return fmt.Sprintf("0x%X", sbox[valueInt>>4][valueInt&0x0F])
}

func InverseSubBytesSubstitution(value string) string {
    sboxInverse := [][]int{
    ...
    valueInt := hexToInt(value)
    return fmt.Sprintf("0x%X", sboxInverse[valueInt>>4][valueInt&0x0F])
}
...
func ShiftRows(state [][]string) [][]string {
    ...
    tempState := make([][]string, len(state))
    for i := range state {
        tempState[i] = make([]string, len(state[i]))
    }
...
func InverseShiftRows(state [][]string) [][]string {
...
func xorOperation(a, b string) string {
    // Perform XOR operation for two hexadecimal values
    aInt := hexToInt(a)
    bInt := hexToInt(b)
    return fmt.Sprintf("0x%X", aInt^bInt)
}
...
func main() {
    arr := []string{"0x63", "0x7c", "0x77", "0x7b", "0xf2"}
    fmt.Println(SubBytesSubstitutionArr(arr))
}